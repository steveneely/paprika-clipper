...
try {
  ...
  assert.ok(await popup.getByRole('button', { name: 'Save this page again' }).isDisabled());
  console.log('PASS popup reopening does not create duplicates');
  await worker.evaluate(() => { globalThis.__responseStatus = -1; });
  await popup.locator('#checked').check();
  await popup.getByRole('button', { name: 'Save this page again' }).click();
  await popup.getByRole('heading', { name: 'Check your recipes.' }).waitFor();
  console.log('PASS unknown network outcome asks user to check Paprika');
